          ...
          <header className="p-4 flex justify-between items-center gap-x-2">
            <Link to="/" className="flex-none focus:outline-none focus:opacity-80" aria-label="Brand">
              <div className="block h-9 w-auto fill-current text-gray-800">
                <span className="text-xl font-bold">AUN Evaluation</span>
              </div>
            </Link>
          ...
          <nav className="h-full overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300">
            <div className="pb-0 px-2 w-full flex flex-col flex-wrap">
              <ul className="space-y-1">
                ...
                {/* Student Portal - Only for students */}
                {isStudent && (
                  <li>
                ...
                {/* Faculty Portal - Only for staff and department heads */}
                {isStaff && (
                  <li>
                    <h3 className="mt-6 mb-2 px-2.5 text-xs font-semibold tracking-wider text-gray-500 uppercase">
                      Faculty Portal
                ...
                {/* Department Head Section - Only for department heads */}
                {isDepartmentHead && (
                  <li>
                    <h3 className="mt-6 mb-2 px-2.5 text-xs font-semibold tracking-wider text-gray-500 uppercase">
                  ...
                  <li>
                    ...
                    <ul className="space-y-1">
                      ...
                      <li>
                        <Link
                          className={`flex items-center gap-x-3 py-2 px-2.5 text-sm rounded-lg hover:bg-gray-100 focus:outline-none focus:bg-gray-100
                            ${isActive('/admin/evaluation-periods') ? 'bg-gray-100 text-gray-800' : 'text-gray-800'}`}
                          to="/admin/evaluation-periods"
                        >
                      ...
                      <li>
                        <Link
                          className={`flex items-center gap-x-3 py-2 px-2.5 text-sm rounded-lg hover:bg-gray-100 focus:outline-none focus:bg-gray-100
                            ${isActive('/department/evaluation-keys') ? 'bg-gray-100 text-gray-800' : 'text-gray-800'}`}
                          to="/department/evaluation-keys"
                        >
                          <Key className="size-4" />
                          Evaluation Keys
                ...
                {/* Staff Section - Only for staff (not department heads or admins) */}
                {user?.role === 'staff' && (
                  <li>
                    <h3 className="mt-6 mb-2 px-2.5 text-xs font-semibold tracking-wider text-gray-500 uppercase">
                      Staff Portal
          ...
          <footer className="mt-auto p-2 border-t border-gray-200">
            <div className="px-2 py-4">
              <div className="flex items-center gap-x-3.5 mb-2">
                ...
                <div>
                  <div className="font-medium text-base text-gray-800">{user?.full_name || 'User'}</div>
                  <div className="text-xs text-gray-500 capitalize">{user?.role?.replace('_', ' ') || 'Guest'}</div>
                </div>
              </div>
